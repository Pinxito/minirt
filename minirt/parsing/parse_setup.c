...
int	setup_scene(t_scene *scene, t_parse_flags *flags, char *file)
{
	...
	if (!flags->has_ambient)
		parse_error(scene, "Missing ambient light (A)");
	if (!flags->has_camera)
		parse_error(scene, "Missing camera (C)");
	if (!flags->has_light)
		parse_error(scene, "Missing light (L)");
	init_camera_basis(&scene->camera);
	if (!init_mlx(scene))
		parse_error(scene, "MinilibX initialization failed");
	return (1);
}
